...
build_testing() {
    ...
    build "$folder" "testing" "-DURITEMPLATE_BUILD_TESTING=ON" \
                              "-DURITEMPLATE_BUILD_DOCS=OFF" \
...
if [[ "$1" == "test" ]] ; then
    ...
    build "build" "docs" "-DURITEMPLATE_BUILD_TESTING=OFF" \
                         "-DURITEMPLATE_BUILD_DOCS=ON"
